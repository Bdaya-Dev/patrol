...
- **Minimum Flutter version**: The minimum supported Flutter version has been
  ...
  introduced to the `flutter_test` package that `patrol` and `patrol_finders_plus`
...
## `patrol_finders_plus` v2

Along with `patrol` v3, we are releasing the v2 of [patrol_finders_plus]. In case you
missed it, we split `patrol_finders_plus` from `patrol` a few months ago in response
...
page][patrol_finders_docs] about `patrol_finders_plus` in case you missed it.

### Changes in `patrol_finders_plus` v2:

- **Minimum Flutter version**: The minimum supported Flutter version of
  `patrol_finders_plus` has been bumped to 3.16, just like in `patrol`'s case.

...
<Steps>
    <Step>
    Install the "Patrol" extension by searching for it in the VS Code Marketplace,
    or in the Open VSX Registry for other VS Code-like editors, eg. Cursor.
...
Let's go through features of the extension, that can help you developing and running Patrol tests.
...
extension. The extension adds a section with Patrol tests.
...
Keep in mind that running and debugging tests use commands from `patrol_cli_plus` under 
the hood, and you still have to have `patrol_cli_plus` installed on your machine - activated 
...
The extension combines Patrol's `develop` command with VS Code's debugging feature - now
 you can debug your tests easier!
...
Since test execution through the extension is using patrol_cli_plus commands under the hood,

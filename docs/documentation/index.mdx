...
<Steps>
    ...
    Install `patrol_cli_plus`:
    ```
      flutter pub global activate patrol_cli_plus
    ```
    ...
    Add a dependency on the `patrol_plus` package in the
    `dev_dependencies` section of `pubspec.yaml`.  `patrol_plus` package requires
    Android SDK version 21 or higher.

    ```
    flutter pub add patrol_plus --dev
    ```
    </Step>
  ...
  ```
    ...
    <Accordion id="ios-setup" title="iOS setup">
      <Steps>
        ...
        <Step id="add-empty-swift-file">
          Add an **empty Swift file** to the `RunnerUITests` target. In Xcode select
            `File > New > File from Template… > Swift File`, name it `RunnerUITests.swift`,
            and make sure only the `RunnerUITests` target is checked. Its contents can be a
            single line:
          ...
          On Xcode 16+/26 the presence of a Swift file makes the linker link the Swift
            runtime for this otherwise pure-Objective-C target. Without it, a Swift-based
            pod (e.g. `google_maps_flutter`, Firebase) that back-deploys the Swift
            compatibility libraries fails to link with
            `Undefined symbols: __swift_FORCE_LOAD_$_swiftCompatibility56`.

          On CI runners where the **Metal toolchain** is the active toolchain (e.g. GitHub
            `macos-26` / Xcode 26), also set `TOOLCHAINS=com.apple.dt.toolchain.XcodeDefault`
            for the build/test step, which pins the default toolchain that still ships those
            libraries.
        </Step>
...
<Accordion id="faq-test-bundle-error" title="Testing fails with error inside 'patrol_test/test_bundle.dart'">
  The reason is probably a mismatch of `patrol` and `patrol_cli_plus` versions. Go to [Compatibility table]
  and make sure that the versions of `patrol` and `patrol_cli_plus` you are using are compatible.
</Accordion>
...
[Patrol CLI]: https://pub.dev/packages/patrol_cli_plus
[FAQ]: /documentation#faq
[Compatibility table]: /documentation/compatibility-table
[README]: https://pub.dev/packages/patrol_cli_plus#installation
[example app]: https://github.com/Bdaya-Dev/patrol/tree/master/packages/patrol_plus/example
[example]: https://github.com/Bdaya-Dev/patrol/blob/master/dev/e2e_app/patrol_test/common.dart
...
[`common.dart`]: https://github.com/Bdaya-Dev/patrol/blob/master/dev/e2e_app/patrol_test/common.dart

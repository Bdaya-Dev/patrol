...
## Logs in `patrol_finders_plus`

By default, enhanced logs are disabled when using `patrol_finders_plus` without the `patrol` package. To enable them, pass the `printLogs: true` argument to the `PatrolTesterConfig` constructor:

...
Since `patrol_finders_plus` is a separate package, referenced in `patrol` package,
...
First, add the `patrol_finders_plus` package as a `dev_dependency` in your app's
`pubspec.yaml`. You can do this by executing the following command in the app's
directory:

```console
flutter pub add patrol_finders_plus --dev
```

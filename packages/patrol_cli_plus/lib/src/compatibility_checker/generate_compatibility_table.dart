...
Future<void> generateCompatibilityTable() async {
  ...
  final buffer = StringBuffer()
    ...
    ..writeln('and `patrol_cli_plus` are compatible with each other.')
    ...
    ..writeln()
    ..writeln(
      'This table shows the compatible versions between patrol_cli_plus and patrol packages.',
    )
    ..writeln()
    ..writeln(
      '| patrol_cli_plus version | patrol version | Minimum Flutter version |',
    )
    ..writeln(
      '|-------------------|----------------|------------------------|',
    );

  // Sort list by patrol_cli_plus version in descending order
  ...
  final rootDir = currentDir.path.endsWith('patrol_cli_plus')

...
import 'package:patrol_cli_plus/src/commands/build.dart';
import 'package:patrol_cli_plus/src/commands/develop.dart';
...
import 'package:patrol_cli_plus/src/coverage/web_coverage_tool.dart';
...
To install a specific version of Patrol CLI, run:
  dart pub global activate patrol_cli_plus <version>
  Example: dart pub global activate patrol_cli_plus 3.5.0''';
  ...
  Future<int?> runCommand(ArgResults topLevelResults) async {
    ...
    if (topLevelResults['version'] == true) {
      _logger.info('patrol_cli_plus v${constants.version}');
      exitCode = 0;
    } else {
  ...
  Future<void> _checkForUpdates({
    ...
    if (patrolVer != null) {
      ...
      if (!isCompatible) {
        ...
        final versionInfo =
            'patrol_cli_plus ${lightCyan.wrap(currentVersion)} is not compatible with patrol ${lightCyan.wrap(patrolVersion)}.';
        ...
        final actionMessage = maxCliVersion != null
            ? 'To resolve this issue, ${currentVersionParsed < maxCliVersion ? "upgrade" : "downgrade"} patrol_cli_plus to a compatible version:'
    ...
    if (patrolVer != null) {
      ...
      if (maxCliVersion != null && latestVersionParsed > maxCliVersion) {
        _showVersionMessage(
          ...
          command: 'dart pub global activate patrol_cli_plus $maxCliVersion',
          additionalWarning:
              "⚠️  Newest patrol_cli_plus $latestVersion is not compatible with your project's patrol version.\nConsider upgrading your patrol package for the latest features.",
        );
        return;
      }
    }

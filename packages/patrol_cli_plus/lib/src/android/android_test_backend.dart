...
import 'package:patrol_cli_plus/src/android/android_test_codegen.dart';
import 'package:patrol_cli_plus/src/android/android_video_recording_manager.dart';
...
import 'package:patrol_cli_plus/src/crossplatform/test_manifest.dart';
import 'package:patrol_cli_plus/src/crossplatform/test_manifest_generator.dart';
          ...
          PatrolLogReader(
              listenStdOut: (onData, {onError, onDone, cancelOnError}) {
                return processLogcat.listenStdOut(
                  (line) {
                    onData(line);
                    final vmDetails = VMConnectionDetails.tryExtractFromLogs(
                      line,
                    );
                    if (vmDetails != null) {
  ...
  Future<void> executeWithoutBuilding(
    ...
    await _disposeScope.run((scope) async {
      ...
      if (!failed &&
        ...
        task.fail('No tests executed for $subject');
        throwToolExit(
          'am instrument succeeded but ZERO tests executed for $subject. The '
          'generated test class matched no tests, or its '
          'testInstrumentationRunner is not pl.leancode.patrol.PatrolJUnitRunner '
          '- see the Android setup steps in the patrol docs. Treating this as a '
          'pass would report a green run that verified nothing.',
        );

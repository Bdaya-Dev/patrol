...
class IOSAppOptions {
  ...
  List<String> testWithoutBuildingInvocation(
    ...
    // is unambiguous and resolves instantly. Real devices are addressed by
    // `platform=iOS,id=<id>` so that device names containing commas cannot
    // break the specifier. `osVersion` (the `--ios` flag) is retained for
    // backward compatibility but is no longer used for simulators.
...
class WebAppOptions {
  ...
  /// Trace mode for Playwright. Valid values: off, on, retain-on-failure,
  ...
  /// Playwright web runner. Unset options are omitted, except
  /// `PATROL_WEB_ERROR_DETECTION`, which is always present as `true`/`false`.
  Map<String, String> toEnvironmentVariables() {
    final values = <String, Object?>{
      ...
      'PATROL_WEB_INIT_TIMEOUT': initTimeout,
      'PATROL_WEB_GREP': grep,
      'PATROL_WEB_GREP_INVERT': grepInvert,
      // Always emitted: the web runner's error gate reads an explicit
      // "true"/"false" rather than treating absence as off.
      'PATROL_WEB_ERROR_DETECTION': errorDetection,
      'PATROL_WEB_ERROR_ALLOW': errorAllow,
      'PATROL_WEB_AUTH_FLOW': authFlow,
      // Emitted as given; the web backend re-resolves these two paths
      // relative to the current working directory before spawning Playwright.
      'PATROL_WEB_AUTH_STATE_FILE': authStateFile,
      'PATROL_WEB_AUTH_FLOW_MODULE': authFlowModule,
    };

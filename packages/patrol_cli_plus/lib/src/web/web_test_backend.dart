...
class WebTestBackend {
          ...
          await _processManager.start(
              ...
              environment: {
                ...
                if (options.authStateFile != null)
                  'PATROL_WEB_AUTH_STATE_FILE': options.authStateFile!,
              },
              runInShell: true,
            )
            ..disposedBy(scope);
  ...
  Future<String> _getWebRunnerPath() async {
    try {
      ...
      final patrolPackage = packageConfig['patrol_plus'];
      if (patrolPackage == null) {
        throw Exception(
          'patrol_plus package not found in package configuration.\n'
          'Please ensure patrol_plus is added as a dependency and run "dart pub get".',
        );
      }

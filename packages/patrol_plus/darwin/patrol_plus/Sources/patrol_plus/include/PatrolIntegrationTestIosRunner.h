...
#define PATROL_INTEGRATION_TEST_IOS_RUNNER(__test_class)                                                            \
  ...
  +(void)launchPatrolAppWithServer : (PatrolServer *)server {                                                       \
    ...
    /* Xcode 26 / iOS 26: launch()'s implicit terminate of the still-registered */                                  \
    /* previous instance fails against a stale/zero pid after the app exits or */                                   \
    /* LLDB detaches. Bring the app to NotRunning ourselves first so launch() */                                    \
    /* has nothing to kill. */                                                                                      \
    if (app.state != XCUIApplicationStateNotRunning) {                                                              \
      [app terminate];                                                                                              \
      NSDate *__patrolTermDeadline = [NSDate dateWithTimeIntervalSinceNow:10.0];                                    \
      while (app.state != XCUIApplicationStateNotRunning && [__patrolTermDeadline timeIntervalSinceNow] > 0) {      \
  ...
  -(void)recordIssue : (XCTIssue *)issue {                                                                          \
    ...
    /* a false green.) */                                                                                           \
    if ([__patrolDesc rangeOfString:@"Failed to terminate [A-Za-z0-9_-]+(\\.[A-Za-z0-9_-]+)+:[0-9]"                 \
  ...
  -(void)recordFailureWithDescription : (NSString *)description                                                     \
    ...
    if ([__patrolFd rangeOfString:@"Failed to terminate [A-Za-z0-9_-]+(\\.[A-Za-z0-9_-]+)+:[0-9]"                   \
                           options:NSRegularExpressionSearch]                                                       \
                .location != NSNotFound ||                                                                          \

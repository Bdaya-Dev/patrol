...
it easy to control the native UI from Dart test code. It must be used together with [`patrol_cli_plus`][patrol_cli_link].
...
[`patrol_finders_plus`](https://pub.dev/packages/patrol_finders_plus).
...
> ⚠️ Additional setup is required to run tests – install [`patrol_cli_plus`][patrol_cli_link] and complete [native setup][docs_setup].

# NOTICE

This product includes software developed by LeanCode — Patrol (https://github.com/leancodepl/patrol), licensed under the Apache License 2.0 (see LICENSE).

Copyright 2021 LeanCode Sp. z o.o.

This is an independent fork maintained by Bdaya-Dev and is not maintained, supported, or endorsed by LeanCode.

Fork point: 41fe088e6e9c98b536d330ba6c12d4af0bfb1189 (upstream master, 2026-05-29).

## Modified files

- CHANGELOG.md (notice in file)
- README.md (notice in file)
- analysis_options.yaml (notice in file)
- android/build.gradle (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/Automator.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/AutomatorServer.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/Logger.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/contracts/AndroidAutomatorServer.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/contracts/Contracts.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/contracts/MobileAutomatorServer.kt (notice in file)
- android/src/main/kotlin/pl/leancode/patrol/contracts/PatrolAppServiceClient.kt (notice in file)
- build.yaml (notice in file)
- darwin/Classes/AutomatorServer/Automator/Automator.swift (notice in file)
- darwin/Classes/AutomatorServer/Automator/IOSAutomator.swift (notice in file)
- darwin/Classes/AutomatorServer/Automator/MacOSAutomator.swift (notice in file)
- darwin/Classes/AutomatorServer/AutomatorServer.swift (notice in file)
- darwin/Classes/AutomatorServer/Contracts.swift (notice in file)
- darwin/Classes/AutomatorServer/IosAutomatorServer.swift (notice in file)
- darwin/Classes/AutomatorServer/MobileAutomatorServer.swift (notice in file)
- darwin/Classes/AutomatorServer/PatrolAppServiceClient.swift (notice in file)
- darwin/Classes/ObjCPatrolAppServiceClient.swift (notice in file)
- darwin/Classes/PatrolIntegrationTestIosRunner.h (notice in file)
- darwin/Classes/PatrolIntegrationTestMacosRunner.h (notice in file)
- darwin/Classes/PatrolPlugin.m (notice in file)
- darwin/patrol_plus.podspec (notice in file)
- example/README.md (notice in file)
- example/analysis_options.yaml (notice in file)
- example/ios/RunnerUITests/RunnerUITests.m (notice in file)
- example/lib/ui/images.dart (notice in file)
- example/lib/ui/widgets/logos_hero.dart (notice in file)
- example/macos/RunnerUITests/RunnerUITests.m (notice in file)
- example/patrol_test/example_test.dart (notice in file)
- example/patrol_test/main_test.dart (notice in file)
- example/pubspec.yaml (notice in file)
- extension/devtools/config.yaml (notice in file)
- lib/patrol.dart (notice in file)
- lib/src/binding.dart (notice in file)
- lib/src/common.dart (notice in file)
- lib/src/custom_finders/patrol_integration_tester.dart (notice in file)
- lib/src/devtools_service_extensions/devtools_service_extensions.dart (notice in file)
- lib/src/native/native_automator.dart (notice in file)
- lib/src/native/native_automator2.dart (notice in file)
- lib/src/native/native_automator_config.dart (notice in file)
- lib/src/platform/android/android_automator.dart (notice in file)
- lib/src/platform/android/android_automator_config.dart (notice in file)
- lib/src/platform/android/android_automator_empty.dart (notice in file)
- lib/src/platform/android/android_automator_native.dart (notice in file)
- lib/src/platform/contracts/android_automator_client.dart (notice in file)
- lib/src/platform/contracts/contracts.dart (notice in file)
- lib/src/platform/contracts/contracts.g.dart (notice in file)
- lib/src/platform/contracts/ios_automator_client.dart (notice in file)
- lib/src/platform/contracts/mobile_automator_client.dart (notice in file)
- lib/src/platform/contracts/patrol_app_service_server.dart (notice in file)
- lib/src/platform/current_io.dart (notice in file)
- lib/src/platform/current_web.dart (notice in file)
- lib/src/platform/ios/ios_automator.dart (notice in file)
- lib/src/platform/ios/ios_automator_config.dart (notice in file)
- lib/src/platform/ios/ios_automator_empty.dart (notice in file)
- lib/src/platform/ios/ios_automator_native.dart (notice in file)
- lib/src/platform/mobile/mobile_automator.dart (notice in file)
- lib/src/platform/mobile/mobile_automator_config.dart (notice in file)
- lib/src/platform/mobile/mobile_automator_native.dart (notice in file)
- lib/src/platform/mobile/patrol_app_service_io.dart (notice in file)
- lib/src/platform/platform_automator.dart (notice in file)
- lib/src/platform/selector.dart (notice in file)
- lib/src/platform/web/patrol_app_service_web.dart (notice in file)
- lib/src/platform/web/web_automator.dart (notice in file)
- lib/src/platform/web/web_automator_empty.dart (notice in file)
- lib/src/platform/web/web_automator_native.dart (notice in file)
- lib/src/platform/web/web_selector.g.dart (notice in file)
- pubspec.yaml (notice in file)
- test/internals_test.dart (notice in file)
- web_runner/.gitignore (notice in file)
- web_runner/eslint.config.js (notice in file)
- web_runner/package-lock.json (cannot carry a comment — listed here)
- web_runner/package.json (cannot carry a comment — listed here)
- web_runner/playwright.config.ts (notice in file)
- web_runner/tests/actions.ts (notice in file)
- web_runner/tests/actions/resizeWindow.ts (notice in file)
- web_runner/tests/contracts.ts (notice in file)
- web_runner/tests/initialise.ts (notice in file)
- web_runner/tests/setup.ts (notice in file)
- web_runner/tests/test.spec.ts (notice in file)
- web_runner/tests/types.ts (notice in file)

## Removed files

- example/assets/image/leancode_logo.svg
- example/assets/image/patrol_logo.svg
- example/ios/Podfile.lock
- example/macos/Podfile.lock
- screenshots/logo.png

<!--
To regenerate this file, run:

    dart run tool/apache_notice/bin/apache_notice.dart --fix
-->
